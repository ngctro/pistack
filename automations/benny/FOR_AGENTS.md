...
i want two pi routines that work together in one slack issue channel.
...
the human enters setup by pointing pi at this file. do not look for or invoke a discovered benny slash skill.
...
for first-time creation, use bundled `/automate` once for triage and once for repro and fix. complete the draft review, approval, readiness check, and Automations native configuration review for the first automation before starting the second.
