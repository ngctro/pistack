...
The human enters setup by pointing pi at the pack's `FOR_AGENTS.md`. The bootstrap flow copies the whole pack into the target repository, then reads this file directly at `.pi/automations/benny/skills/setup-benny/SKILL.md`.

Benny needs external configuration and two live pi routines.
...
Do this before asking for Benny configuration and before invoking the bundled `/automate` skill.
...
Use only model slugs shown as available in `pstack_models`. Do not guess a slug and do not carry over a private default.
...
Prefer configured MCP Slack tools for reads and posts. The optional `BENNY_SLACK_BOT_TOKEN` may fill a narrow gap such as editing one operations status message or downloading an attachment. Store the value in a secret manager or environment, not in YAML.
...
5. Read and follow the bundled `automate` skill.
...
8. Let `automate` show its draft table, obtain approval, ask readiness, and save a disabled native routine.
9. Finish the native configuration review for this automation before starting the next one.
...
After the triage native configuration review is complete, give `automate` this complete repro and fix intent:
...
Do not duplicate `automate`'s Slack, repository, integration, completeness, authentication, draft-review, approval, readiness, or native configuration review work.

### Existing automations

For existing routines use `pstack_routines list`, then the bundled `automate` review flow to update the existing name. Saving an update disables it until readiness is rechecked.

Finish configuration, routing, control-adapter, and feature-map validation. Then give the user this concise configuration checklist.
...
Ask the user to update each existing automation directly in its native routine configuration. Do not create replacements or duplicates.

### Creation boundary

Use the bundled `automate` skill and `pstack_routines` for reviewed native setup. Never call private Cursor services or construct proprietary deep links. Save disabled configurations, obtain approval, and pass readiness checks before enabling.

Do not enable either automation until the thread-safety test passes after the routine save.
