...
The [Opening a PR playbook](../../skills/poteto-mode/playbooks/opening-a-pr.md) runs `/skill:deslop` on the diff before each commit and applies [`/skill:unslop`](../../skills/unslop/SKILL.md) to the PR description and commit bodies. `/skill:deslop` ships in the bundled team-kit port, not in pstack. If you don't have it, ask for the same outcome in plain words: remove narrating comments, unsupported guards, dead compatibility paths, and unrelated edits.
