...
Use `/pstack-agents` or `pstack_workers` with `action: "agents"` to discover named agent definitions. Custom Markdown agents load from user directories and trusted project `.pi/agents` or `.cursor/agents`. Pass the name as `subagent_type`. Agent metadata controls model, read-only, and background defaults. Explicit task model or role wins over the agent model. A read-only definition cannot be weakened by a false task flag. See the [custom agent contract and Cursor capability map](cursor-parity.md) for precedence, supported metadata, and limitations.

Call `pstack_task` multiple times in one assistant turn to fan out. Background mode is the default. Each call returns an ID, working directory, report path, and saved pi session path. `pstack_workers` lists, waits, interrupts, cancels, and resumes. Completed background workers send a display notification without triggering a turn. Parent sees it on next turn or via `pstack_workers` list/wait. A resume needs a consolidated brief; do not rely on earlier interrupts.

Workers inherit the parent's current model and thinking level unless an agent definition, role, or explicit model overrides the model. Resume preserves the saved exact model, read-only flag, and `instructions.md` snapshot without rereading the agent source. Missing saved instructions fail explicitly. Referenced files remain live, and thinking level still comes from the current parent. They use the installed pi CLI and its provider credentials. Configure provider-extension-only models globally so child pi processes can load them too. A failed or unavailable model is an error, not a silent substitution.
...
Benny stays dormant in `automations/benny`. The native `/skill:automate` skill replaces the proprietary automation editor flow. `pstack_routines` creates disabled routine configs, and the local Node runner supplies authenticated webhooks, Slack Events API verification/deduplication, scheduled triggers, and durable run artifacts. Read [automation hosting](automations.md) before enabling any routine. No Slack credentials or hosted Cursor services are included.
