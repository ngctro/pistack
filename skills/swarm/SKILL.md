...
4. Pick the worker model from `swarm workers` in `~/.pi/agent/pstack.json` when present. Otherwise use `inherit-parent`. For a model race, name each arm's model up front.
