...
Call `pstack_history` to find the exact workspace's saved pi sessions. Read the returned JSONL paths. The first line is session metadata; message, compaction, and extension-state entries follow. Never construct a Cursor-style slug or assume every line is a message.
