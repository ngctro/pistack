...
**Control skill.** Pick it by surface. Browser, Electron, and web UIs use `control-ui` from the bundled team-kit port. CLIs and TUIs use `control-cli` from the bundled team-kit port. Native mobile uses whatever simulator-driving skill the repo has. A PR that touches two surfaces gets lanes on both. A surface with no control skill is a risk in Appendix C, and its live block still names how each lane drives it.
...
- [ ] Read these at program start and every tick. Resolve bundled files from the installed package root; refresh pistack in a separate checkout when latest upstream instructions are needed. Only the project's own control skill comes from the project's trunk.
  - [ ] `skills/poteto-mode/playbooks/<execution playbook>.md`
  - [ ] `skills/swarm/SKILL.md`
  - [ ] `git show origin/main:<project control skill path>`
  - [ ] `skills/poteto-mode/playbooks/opening-a-pr.md`
  - [ ] `skills/<each other leaf skill the program uses>`
- [ ] Arm the 30-minute audit tick. Use `pstack_loop` with `seconds: 1800` in a persistent pi TUI/RPC session. Keep pi alive with tmux or a service. Never leave the cadence to memory.
...
Each live lane runs on its own cloud VM at the PR head. Drive through `control-ui` or `control-cli` from the bundled team-kit port.
