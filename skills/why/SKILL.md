...
Before spawning investigators, call `pstack_tools` to list installed pi tools and `pstack_mcp` with `action: servers` to list configured integrations. Inspect each server's tools, resources and schemas before choosing a category. Follow pagination cursors. Missing authentication is an explicit evidence gap, not permission to invent an endpoint.
...
- `readonly`: `false` for git/gh or integrations whose read tools lack read-only annotations. Read-only pi workers retain `pstack_mcp` but restrict calls to tools annotated `readOnlyHint: true`. Investigators must not mutate files or external state; agent mode is a posture, not a sandbox.
...
- `readonly`: `false` (agent mode) when citation checks need git/gh or unannotated MCP tools. The brief forbids writes. Use readonly when all required tools support it.
