...
Locate the active workspace's transcripts before fanning out. The system prompt names the workspace's the workspace sessions directory returned by `pstack_history` directory. Use only that path. Don't glob across unrelated workspace session directories. That crosses workspace boundaries and reads private chats from unrelated projects.
...
Mining misses intent that hasn't come up yet. Use the `pstack_ask` tool (structured multi-choice) rather than asking the user to type from scratch.
